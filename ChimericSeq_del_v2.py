...
@author: JBSUSER
 ...
 ChimericSeq is distributed in the hope that it will be useful,
...
#---------------------------------------------------------------------------------------------------
# Core class of ChimericSeq
# Set up operation directories and default settings
# This Class also includes routines for:
#  -- build host index,
#  -- build viral index,
#  -- host alignment,
#  -- viral alignment,
#  -- create SAM file for fost,
#  -- create SAM file for viral,
#  -- map viral SAM and host SAM,
#  -- split large file, &
#  -- main "run" 
#---------------------------------------------------------------------------------------------------
class Core:
    ...
    def createInterface(self):
        self.topLevelTK=tk.Tk()
        self.topLevelTK.withdraw()
        root=tk.Toplevel(self.topLevelTK)
    ...
    #---------------------------------------------------------------------------------------------------
    # reet flags & variables used by processing splited files
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # load GTF file and build Gene data
    #---------------------------------------------------------------------------------------------------
    def loadGTF(self):
    ...
    #---------------------------------------------------------------------------------------------------
    # print log on the console and append data to list for future save purpose
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # display default settings on the console
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # ask the user to input the filenames of for test
    # If "Select Directory" is checked, that means the user want to proces splited large file.
    # For processing splited files, user need to specify the folder which contains smaller size files.
    #---------------------------------------------------------------------------------------------------
    def getBatchReads(self):
        if self.app.selectDirVar.get() == 1 or self.splitrun == True: 
        ...
        if (self.getProcessingLargeFileStatus() == False) and (self.processLargeFileIndex == 0):
            temp=filedialog.askdirectory(title='Select Batch Reads Directory',initialdir=self.workingDirectory+'/Reads')
            if temp:
                self.readDir=temp
                self.processLargeFileDir = temp
                self.printToLog('Directory Selected:\n'+self.readDir)
                self.paired=True
                for i in range(0, self.batchfileslength):
                    if i%2 == 0 and self.batchfiles[i][-4:-3] != '1':
                        self.paired = False
                    elif i%2 == 1 and self.batchfiles[i][-4:-3] != '2':
                        self.paired = False
                if self.paired == True:
                    self.printToLog('Paired = True')
                else:
                ...
                self.printToLog(str(self.batchfileslength) + ' files total')
                return self.batchfiles
            else:
                return 'break'
    ...
    def getReads(self):
        ...
        if self.splitrun == True:
            if (self.getProcessingLargeFileStatus() == False) and (self.processLargeFileIndex == 0):
                temp=self.directory
                if temp:
                    self.readDir = temp
                    self.processLargeFileDir = temp
                    self.printToLog('Split and Run Directory Selected:\n'+temp)
                    self.paired=True
                    try:
                        ...
                        try:
                            ...
                            self.printToLog('\n------- Error!!! ----------------------------')
                            self.printToLog('cannot locate split information file '+filename+".")
                            self.printToLog('---------------------------------------------\n')
                            return 'break'
                        fileCount=splitInfoFile.readline().replace("\n","")
                        self.printToLog("splitrun fileCount = "+str(fileCount))
                        ...
                        if "????" in file2: #if only file1, then non-paired?
                            self.splitFile2 = None
                            self.paired=False
    ...
    #---------------------------------------------------------------------------------------------------
    # check if Bowtie2 has been installed
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # check if host index files and viral index files exist
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # Ask if the user would like to build viral index files
    #---------------------------------------------------------------------------------------------------
    ...
    def buildBowtie2ViralIndex(self,prefix):    
        def target():
            try:
                ...
                filename='"'+self.ViralRefFa+'"' # put path name inside '"' to allow space as part of path name
                if self.platformType == "Windows":                
                    temp=subprocess.Popen([self.Bowtie2Folder+'/bowtie2-build.bat',filename,prefix],stdout=subprocess.PIPE,bufsize=1)#,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
                else:
                    filename=self.ViralRefFa # Mac can't put path name inside '"' 
                    temp=subprocess.Popen([self.Bowtie2Folder+'/bowtie2-build',filename,prefix],stdout=subprocess.PIPE,bufsize=1)#,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)    
                self.app.update()
    ...
    #---------------------------------------------------------------------------------------------------
    # Save all the settings for future use
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # Split large file.
    # Due to the resource limitation, it will be better to split large file into several smaller files.
    # The new smaller files will have up tp 1,000,000 reads in each file.
    #---------------------------------------------------------------------------------------------------
    def splitLargeFile(self, SplitAndRun=None):
        self.file1=None
        self.file2=None
        def printInfo(info):
            self.splitLargeFileOutText.configure(state=tk.NORMAL)
            self.splitLargeFileOutText.insert(tk.END, info+"\n")
            self.splitLargeFileOutText.see(tk.END)
            self.splitLargeFileOutText.configure(state=tk.DISABLED)
        ...
        def startSplitFiles():
            ...
            self.splitLargeFileBttn2.configure(state=tk.DISABLED) # prevent spliting again in the middle of process
            ...
            readsCount = 1000000
            try:
                f_c = open("_splitFileCount.dat", "r")
                size_count=int(f_c.readline().replace("\n",""))
                if (size_count < 10000) or (size_count > 10000000):
                    size_count = 1000000
                readsCount = size_count
                f_c.close()
            except:
                readsCount = 1000000
            printInfo("Splitting file size = "+str(readsCount))                            
            ...
            if (file2 != None):
                if pathName.lower().endswith("fastq"):
                    ...
                    a=str(pathName[:-4])
                    a0=str(pathName[:-4])
            else: #non-paired mode
                if pathName.lower().endswith("fastq"):
                    newDirName=pathName[:-6]+"_split"
                    a=str(pathName[:-6])
                    a0=str(pathName[:-6])
                else:
                    newDirName=pathName[:-3]+"_split"
                    a=str(pathName[:-3])
                    a0=str(pathName[:-3])

            newDirNameFull=dirName+"/"+newDirName
            self.directory = newDirNameFull
            ...
            printInfo("=====================================================================")
            if self.splitrun == True:
                closeSplitLargeFileWin()
                self.getReads()
            return
        
        def readLargeFilles():
            if self.batchmode == True:
                temp = self.readDir[self.batchindex:self.batchindex+2]
                self.printToLog( temp )
            ...
            self.splitLargeFileBttn2.configure(state=tk.NORMAL)
            return
        ...
        if (SplitAndRun != True):
            ...
            self.printToLog('split and Run LargeFile!')
        self.splitLargeFileWin=tk.Toplevel(self.app.master)
        if (SplitAndRun != True):
            self.splitLargeFileWin.title('Split large fastq files')
        else:
            self.splitLargeFileWin.title('Split and run large fastq files')
        self.splitLargeFileWin.geometry('620x620')   
        self.splitLargeFileWin.protocol('WM_DELETE_WINDOW',closeSplitLargeFileWin)
        self.labelSplit=tk.Text(self.splitLargeFileWin,height=1,width=20,state=tk.DISABLED,relief=tk.FLAT,background=self.app.defaultbg)
        self.app.changeLabel(self.labelSplit,'Fastq File(s):')
        self.labelSplit.grid(row=0,column=0,columnspan=1,sticky=tk.W)
        
        self.splitLargeFileFrame1=tk.Frame(self.splitLargeFileWin)
        self.splitLargeFileFrame1.grid(row=1,column=0,sticky='nw',columnspan=2)         
        self.splitLargeFileBttn1=tk.Button(self.splitLargeFileFrame1,text = '. . .', command=readLargeFilles)
        self.splitLargeFileBttn1.grid(row=0,column=0,sticky='',ipadx=3,ipady=5)
        self.splitLargeFileFastqBox=tk.Text(self.splitLargeFileFrame1,height=2,width=70,state=tk.DISABLED,background=self.app.defaultbg)
        self.splitLargeFileFastqBox.grid(row=0,column=1,sticky=tk.W,columnspan=1)
        ...
        if (SplitAndRun != True):
            self.splitLargeFileBttn2=tk.Button(self.splitLargeFileFrame1,text = ' Split Files ', state = 'disabled', command=startSplitFiles)
        else:
            self.splitLargeFileBttn2=tk.Button(self.splitLargeFileFrame1,text = ' Split and Run Files ', state = 'disabled', command=startSplitFiles)
        self.splitLargeFileBttn2.grid(row=1,column=1,sticky='',ipadx=3,ipady=5)

        self.splitLargeFileOutText=tk.Text(self.splitLargeFileFrame1,height=30,width=70,state=tk.DISABLED, background=self.app.defaultbg)
    ...
    def splitlargefileBATCH(self):
        self.file1=self.batchfilesloc + '/' + self.batchfiles[self.batchindex]
        self.file2=self.batchfilesloc + '/' + self.batchfiles[self.batchindex + 1]

        start_time=time.time()
        file1=self.file1
        file2=self.file2

        self.printToLog("Splitting Files...")            
        self.printToLog(file1)
        if (file2 != None):
            self.printToLog(file2)
        ...
        try:
            ...
            readsCount = 1000000
        self.printToLog("Splitting file size = "+str(readsCount))                            
        ...
        newDirNameFull=dirName+"/"+newDirName
        self.directory = newDirNameFull
        self.printToLog(newDirNameFull)

        try:
            os.mkdir(newDirNameFull)
        except FileExistsError:
            if os.path.exists(newDirNameFull):
                self.printToLog("path exist already")
            else:
                self.printToLog("create failed")
                return

        showReadsCount = readsCount / 5 
        self.printToLog('Reading '+file1)
        file_count1=1
        b="_"+str(file_count1)
        c='_1.fastq'
        filename=newDirNameFull+"/"+a+b+c
        self.printToLog("creating file: "+filename)
        handle = open(filename, "w")
        cnt = 0
        for (title, sequence, quality) in FastqGeneralIterator(open(file1)) :
            handle.write("@"+title+"\n")
        ...
        self.printToLog("=====================================================================")
        self.printToLog("filename1 format = "+filename1_format)
        if file2 != None:
            self.printToLog("filename2 format = "+filename2_format)
        else:
            filename2_format="????"
            file_count2=0
        out=open(newDirNameFull+"/"+self.splitInfoFilename,"w")
        ...
        out.write(filename2_format+"\n")
        out.close()
        self.printToLog(" new folder '"+newDirNameFull+"' with "+str(file_count1+file_count2)+" files created")
        self.printToLog(" time used = "+str(end_time-start_time)+" seconds")
        self.printToLog("========================    Split Complete   ========================")
        self.printToLog("=====================================================================")
        if self.splitrun == True:
            self.getReads()
        return
    
    def splitrunlargefile(self):
        self.splitrun = True
        if (self.ViralRefFa == 'None Selected'):
            self.printToLog('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            self.printToLog('You must first load config or set locations.')
            self.printToLog('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            return
        elif self.batchmode == True:
            self.splitlargefileBATCH()
        else:
            self.splitLargeFile(SplitAndRun=True)
    ...
    #---------------------------------------------------------------------------------------------------
    # Ask if the user would like to build host index files
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # Build host index files
    #---------------------------------------------------------------------------------------------------
    def buildBowtie2HostIndex(self,prefix): 
        def target():
            try:
                self.app.update()
                filename='"'+self.HostRefFa+'"' # put path name inside '"' to allow space as part of path name
                os.chdir(str(self.HostRefFolder))
                if self.platformType == "Windows":
                    temp=subprocess.Popen([self.Bowtie2Folder+'/bowtie2-build.bat',filename,prefix],stdout=subprocess.PIPE,bufsize=1)#,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
                else:
                    filename=self.HostRefFa # Mac can't put path name inside '"' 
                    temp=subprocess.Popen([self.Bowtie2Folder+'/bowtie2-build',filename,prefix],stdout=subprocess.PIPE,bufsize=1)#,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)                
                self.app.update()
                for line in iter(temp.stdout.readline,b''):
                    self.printToLog(line.decode('utf-8'))
    ...
    #---------------------------------------------------------------------------------------------------
    # Routines used by procesing large file (splited files)
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # Callback for "Start Run" button 
    #---------------------------------------------------------------------------------------------------
    def decidenextstep(self):
        """
        if (self.ViralRefFa == 'None Selected') and (self.batchmode == True):
            self.printToLog('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            self.printToLog('You must first load config or set locations.')
            self.printToLog('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            return
        ...
        if self.batchmode == False:
            ...
            self.batchindex = 0
            self.splitrunlargefile()
        elif self.batchmode == True and self.batchindex < self.batchfileslength - 2:
            self.batchindex += 2
            self.printToLog('\n======================================================================\n')
            self.printToLog(self.readBaseName + ' completed, now starting ' + self.batchfiles[self.batchindex])
            self.printToLog('\n======================================================================\n') 
            self.resetLargeFileProcess()
            self.splitlargefileBATCH()
        elif self.batchmode == True and self.batchindex == self.batchfileslength - 2:
            self.printToLog('\n======================================================================\n')
            self.printToLog('\n======================================================================\n')
            self.printToLog('ALL FILES COMPLETED!')
            self.printToLog('\n======================================================================\n')
            self.printToLog('\n======================================================================\n')
            self.app.selectDirButton.configure(state=tk.DISABLED)
            self.app.selectBatchButton.configure(state=tk.DISABLED)
            self.app.bttnStart.configure(state=tk.DISABLED)    
            return
            ...
            if self.getProcessingLargeFileStatus()==False:
                ...
                if (self.processLargeFileFirstRead):
                    ...
                    if temp=='break':
                        ...
                        self.changeProcessingLargeFileStatus(False)
                        self.changeProcessingLargeFileCompleteStatus(True)
                        self.app.selectDirButton.configure(state=tk.NORMAL)
                        self.app.bttnStart.configure(state=tk.NORMAL)
    ...
    #---------------------------------------------------------------------------------------------------
    # Check if reads file has been specified
    #---------------------------------------------------------------------------------------------------
    def checkReads(self):
        if self.processLargeFile == True:
    ...
    #---------------------------------------------------------------------------------------------------
    # Specify the location of Python and Perl
    #---------------------------------------------------------------------------------------------------
    def addPaths(self):
        if self.platformType == "Windows":
            try:
    ...
    #---------------------------------------------------------------------------------------------------
    # Create SAM file for viral and host
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # Count unique reads for high reads count.
    # Check for duplicate reads could be a n*n complexity.
    # This routine do a sorting before the duplicate check.
    # This routine also check for high similarity reads.
    #---------------------------------------------------------------------------------------------------
    def countUniqueForHighCount(self, countUniqueOnly=False):
        ...
        for i in range(len(self.Map.data)):
            ...
            out.write('\n')
        out.close()
        self.printToLog('total '+str(unique_count)+' reads saved.')
        
        self.printToLog('save all unique reads for debug purpose............')
        new_list_data3=sorted(new_list_data2, key=lambda item:item[0]) # sorting on sequence length
        out=open(self.runDirectory+'/'+self.readBaseName+'_DEBUG_unique_'+st+'.csv','w')
        for a in attributeList_debug:
            out.write(a+',')
        out.write('\n')
        for i in range(0, len(new_list_data3)):
            if new_list_data3[i][5]==True:
                ndx=int(new_list_data3[i][0])
                for item in attributeList_debug:
                    out.write('"'+str(self.Map.data[ndx][item])+'"'+',')
                out.write('\n')
        out.close()
        self.printToLog('total '+str(unique_count)+' unique reads saved.')
        """    
        """
    ...
    #---------------------------------------------------------------------------------------------------
    # Count unique and low similarity reads 
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # Save final reads for multiple files.
    # For multiple files, identified reads will be saved automatically when all files have
    #     been processed. 
    #---------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------
    # Save identified reads & log
    # the duplicate reads and high similarity reads will not be saved.
    #---------------------------------------------------------------------------------------------------
    def saveData(self):
                ...
                if self.DEBUG==1:
                    ...
                    if not(self.processLargeFile) or (self.processLargeFile):
                        ...
                        for i in range (0,len(self.Map.data)):
                            ...
                            out.write('\n')
                        out.close()
                        self.printToLog('total '+str(len(self.Map.data))+' reads saved.')
                
                firstSave=False
                if (self.processLargeFile): # handle large file
                    if len(self.savedReads)==0 and (self.savedReadsDir != self.processLargeFileDir):
                        firstSave=True
...
#---------------------------------------------------------------------------------------------------
# Mapper class
# This class is to get all the detail information for each identifed read
#---------------------------------------------------------------------------------------------------
class Mapper:
    ...
    def gatherAligned(self):
        ...
        if (self.core.app.mh_stringency.get(1.0,'end-1c').strip() == ""): # microhomology stringency % is empty
            ...
            microhomology_nt = 0
        self.core.app.listBox1.delete(0,tk.END)
        ...
        for i in range (0,len(self.hSam.sam)):
            ...
            if not stop:
                ...
                self.core.app.listBox1.insert(tk.END,name)
                self.readCount+=1
    ...
    #---------------------------------------------------------------------------------------------------
    # Load read into display list.
    # Each read will have information including:
    #  'ReadName','ReadLength','Sequence','Chromosome','Gene','InsideGene','DistanceToGene',
    #  'GeneDirection','Focus','GeneObj','FocusObj','HostLocalCords','Hlength','HostRefCords',
    #  'HostOrientation','ViralAccession','ViralLocalCords','Vlength','ViralRefCords','ViralOrientation',
    #  'HTM','HTMAdjusted','VTM','VTMAdjusted','Overlap','OverlapTM','OverlapTMAdjusted','Inserted',
    #  'Microhomology','Filtered','Similarity','HostMapFlag','ViralMapFlag','HMapQ','VMapQ',
    #  'FastqSource','Index'
    #---------------------------------------------------------------------------------------------------
    def loadData(self,number, microhomology_percent, microhomology_nt):
        ...
        data['HostRefCords']=[int(self.hSam.sam[number][3]),int(self.hSam.sam[number][3])+hlen-1]
        data['ViralRefCords']=[int(self.vSam.sam[n][3]),int(self.vSam.sam[n][3])+vlen-1]
...
#---------------------------------------------------------------------------------------------------
# Class for GUI window and associed callback routines
#---------------------------------------------------------------------------------------------------
class Interface(tk.Frame):
    ...
    def create_widgets(self):
        self.master.title("ChimericSeq "+ self.releaseVersion)

        self.label1=tk.Text(self.master,height=1,width=20,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.label1,'Fastq File(s):')
        self.label1.grid(row=0,column=0,columnspan=1,sticky=tk.W)
        self.selectDirVar = tk.IntVar()
        self.selectDirButton = tk.Checkbutton(self.master, text = "Select Directory", variable=self.selectDirVar, onvalue=1, offvalue=0, command=self.selectDirToggle)
        self.selectDirButton.grid(row=0,column=1,columnspan=1,sticky=tk.W)
        self.selectBatchVar = tk.IntVar()
        self.selectBatchButton = tk.Checkbutton(self.master, text = "Batch Mode", variable = self.selectBatchVar, onvalue = 1, offvalue = 0, command = self.selectBatchToggle)
        self.selectBatchButton.grid(row = 0, column = 1, columnspan = 1)
        
        self.frame1=tk.Frame(self.master)
        self.frame1.grid(row=1,column=0,sticky='nw',columnspan=2)         
        self.bttn1=tk.Button(self.frame1,text = '. . .', command=self.getReads)
        self.bttn1.grid(row=0,column=0,sticky='',ipadx=3,ipady=5)
        self.fastqBox=tk.Text(self.frame1,height=2,width=70,state=tk.DISABLED,background=self.defaultbg)
        self.fastqBox.bind('<Control-F1>',self.selectSplitFiles)
        self.fastqBox.grid(row=0,column=1,sticky=tk.W,columnspan=1)
        ...
        self.label2=tk.Text(self.master,height=2,width=65,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.label2,'Reads Containing\nChimeric Sequences:')
        self.label2.grid(row=2,column=0,columnspan=2,sticky=tk.W)

        self.listBoxFrame=tk.Frame(self.master)
        self.listBoxFrame.grid(row=3,column=0,columnspan=1,rowspan=1,sticky='wens')
        self.listBox1=tk.Listbox(self.listBoxFrame,selectmode=tk.SINGLE,height=20,width=25,activestyle=tk.NONE)
        self.listBox1.grid(row=0,column=0,sticky='wens')
        self.scrollbar1=ttk.Scrollbar(self.listBoxFrame,command=self.listBox1.yview)
        self.scrollbar2=ttk.Scrollbar(self.listBoxFrame,command=self.listBox1.xview,orient=tk.HORIZONTAL)
        ...
        self.ysb=ttk.Scrollbar(self.statsFrame,orient=tk.VERTICAL, command= self.statsTree.yview)
        self.xsb=ttk.Scrollbar(self.statsFrame,orient=tk.HORIZONTAL, command= self.statsTree.xview)
        ...
        self.seqFrame=tk.Frame(self.master)
        self.seqFrame.grid(row=0,column=2,ipadx=0,rowspan=3,columnspan=5,sticky='nw')
        self.seqLabel=tk.Text(self.seqFrame,height=1,width=56,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        ...
        self.seqBox=tk.Text(self.seqFrame,height=4,width=70,state=tk.DISABLED,relief=tk.SUNKEN)
        ...
        self.log=tk.Text(self.logFrame,height=20,width=70,state=tk.DISABLED)
        ...
        self.logoL=tk.Text(self.logoFrame,height=1,width=45,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.logoL,'')
        self.logoL.grid(row=0,column=0,columnspan=1,sticky=tk.W)
        self.logoCanvas=tk.Canvas(self.logoFrame)
        self.logo=tk.PhotoImage(file=self.c.workingDirectory+'/JBS.gif')
        self.logoCanvas.create_image(0,0,image=self.logo,anchor=tk.NW)
        ...
        self.buttonL=tk.Text(self.buttonFrame,height=1,width=15,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.buttonL,'Controls:')
        self.buttonL.grid(row=0,column=0,columnspan=2,sticky=tk.W)
        global btn_text
        btn_text = tk.StringVar()
        btn_text.set('Start Run')
        self.bttnStart = tk.Button(self.buttonFrame, textvariable = btn_text, command=self.c.decidenextstep)
        # self.bttnStart.pack()
        self.bttnStart.grid(row=1,column=0,columnspan=2,sticky='nesw')
        self.yesbttn=tk.Button(self.buttonFrame,text='Yes',command=hello)
        self.yesbttn.grid(row=2,column=0,sticky='nwes')
        self.nobttn=tk.Button(self.buttonFrame,text='No',command=hello)
        self.nobttn.grid(row=2,column=1,sticky='nwes')
        
        
        self.modeIntVar=tk.IntVar()
        self.menubar = tk.Menu(self.master)
        self.filemenu = tk.Menu(self.menubar, tearoff=0)
        self.modeMenu=tk.Menu(self.filemenu,tearoff=0)   
        ...
        self.filemenu.add_command(label='Split and Run Large File', command = self.c.splitrunlargefile)
        self.filemenu.add_separator()
        self.filemenu.add_command(label="Exit", command=self.EXIT)
        self.menubar.add_cascade(label="File", menu=self.filemenu)
        self.editmenu = tk.Menu(self.menubar, tearoff=0)
        self.cpumenu = tk.Menu(self.editmenu,tearoff=0)
        self.cpuIntVar=tk.IntVar()
        self.autoModeVar=tk.IntVar()
        self.autoModeVar.set(1)  ##set to 1 for final code
        self.loadAlignmentBoolVar=tk.IntVar()
        ...
        self.otempFilter=tk.IntVar()
        self.otempFilter.set(0)
        self.olenFilter=tk.IntVar()
        self.olenFilter.set(0)
        self.htempFilter=tk.IntVar()
        self.htempFilter.set(0)
        self.hlenFilter=tk.IntVar()
        self.hlenFilter.set(0)
        self.vtempFilter=tk.IntVar()
        self.vtempFilter.set(0)
        self.vlenFilter=tk.IntVar()
        self.vlenFilter.set(0)
        self.nomicrohomologyFilter=tk.IntVar()
        self.nomicrohomologyFilter.set(0) 
        self.doGTFBoolVar=tk.IntVar()
        self.doGTFBoolVar.set(1) 
        self.doReadCleanBoolVar=tk.IntVar()
        self.doReadCleanBoolVar.set(0) 
        self.filtermenu = tk.Menu(self.editmenu,tearoff=0)
        ...
        self.treeMenu=tk.Menu(self.statsFrame,tearoff=0)
        ...
        self.clipLabel=tk.Text(self.OptionsWin,height=1,width=40,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.clipLabel.grid(row=0,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.clipLabel,'Clipped Sequence Min Length (0 - 50):')
        self.clipText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.clipText.insert(tk.END,self.c.CSminLen)
        self.clipText.grid(row=0,column=1,columnspan=1,sticky=tk.W)
        
        self.saltLabel=tk.Text(self.OptionsWin,height=1,width=40,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.saltLabel.grid(row=1,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.saltLabel,'Salt Concentration (mM) (0 - 500):')
        self.saltText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.saltText.insert(tk.END,self.c.Na)
        self.saltText.grid(row=1,column=1,columnspan=1,sticky=tk.W)
        
        self.TrimmingLabel=tk.Text(self.OptionsWin,height=1,width=40,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.TrimmingLabel.grid(row=2,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.TrimmingLabel,'Trimming 5\' (bp) (0 - 70):')
        self.TrimmingText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.TrimmingText.insert(tk.END,self.c.Trim)
        self.TrimmingText.grid(row=2,column=1,columnspan=1,sticky=tk.W)
        
        self.TrimmingLabel2=tk.Text(self.OptionsWin,height=1,width=40,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.TrimmingLabel2.grid(row=3,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.TrimmingLabel2,'Trimming 3\' (bp) (0 - 70):')
        self.TrimmingText2=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.TrimmingText2.insert(tk.END,self.c.Trim2)
        self.TrimmingText2.grid(row=3,column=1,columnspan=1,sticky=tk.W)

        self.DistanceLabel=tk.Text(self.OptionsWin,height=1,width=40,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.DistanceLabel.grid(row=4,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.DistanceLabel,'Gene Distance Threshold (0 - 1000000):')
        self.DistanceText=tk.Text(self.OptionsWin,height=1,width=7,state=tk.NORMAL)
        ...
        self.DistanceText.insert(tk.END,self.c.distanceThreshold)
        self.DistanceText.grid(row=4,column=1,columnspan=1,sticky=tk.E+tk.W)

        self.SimilarityLabel=tk.Text(self.OptionsWin,height=1,width=40,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.SimilarityLabel.grid(row=5,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.SimilarityLabel,'Similarity Max% (80 - 100):')
        self.readSimilarityText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.readSimilarityText.insert(tk.END,self.c.readSimilarity)
        self.readSimilarityText.grid(row=5,column=1,columnspan=1,sticky=tk.W)
        
        self.StretchLabel=tk.Text(self.OptionsWin,height=1,width=40,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.StretchLabel.grid(row=6,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.StretchLabel,'nt Stretch Count Max (5 - 30):')
        self.ntStretchCountText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.ntStretchCountText.insert(tk.END,self.c.ntStretchCount)
        self.ntStretchCountText.grid(row=6,column=1,columnspan=1,sticky=tk.W)

        self.UnidentifyLabel=tk.Text(self.OptionsWin,height=1,width=40,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.UnidentifyLabel.grid(row=7,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.UnidentifyLabel,'nt Unidentified Count Max (5 - 25):')
        self.ntUnidentifiedCountText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.ntUnidentifiedCountText.insert(tk.END,self.c.ntUnidentifiedCount)
        self.ntUnidentifiedCountText.grid(row=7,column=1,columnspan=1,sticky=tk.W)
        
        
        self.mh_stringency=tk.Text(self.OptionsWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.mh_stringency.grid(row=8,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.mh_stringency,'Microhomology Max% (80 - 100):')
        self.mh_stringency=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.mh_stringency.grid(row=8,column=1,columnspan=1,sticky=tk.W)
        
        self.mh_stringency1=tk.Text(self.OptionsWin,height=1,width=31,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.mh_stringency1.grid(row=9,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.mh_stringency1,'Microhomology nt (15 - 35):')
        self.mh_stringency1=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.mh_stringency1.grid(row=9,column=1,columnspan=1,sticky=tk.W)
        
        self.printToLog('===== set microhomology')
        if (self.c.microhomologystringency_selection == 0):
            self.mh_stringency.insert(tk.END,self.c.microhomologystringency)
            self.mh_stringency1.insert(tk.END,"")
        else:
            self.mh_stringency1.insert(tk.END,self.c.microhomologystringency1)
            self.mh_stringency.insert(tk.END,"")
        
        self.tempFilterLabel=tk.Text(self.OptionsWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.tempFilterLabel.grid(row=10,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.tempFilterLabel,'Overlap TM Max (C) (0 - 95):')
        self.tempFilterText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.tempFilterText.insert(tk.END,self.c.otempFilter)
        self.tempFilterText.grid(row=10,column=1,columnspan=1,sticky=tk.W)
        
        self.lenFilterLabel=tk.Text(self.OptionsWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.lenFilterLabel.grid(row=11,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.lenFilterLabel,'Overlap Length Max (0 - 100):')
        self.lenFilterText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.lenFilterText.insert(tk.END,self.c.olenFilter)
        self.lenFilterText.grid(row=11,column=1,columnspan=1,sticky=tk.W)
        
        self.htempFilterLabel=tk.Text(self.OptionsWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.htempFilterLabel.grid(row=12,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.htempFilterLabel,'Host TM Min (C) (0 - 80):')
        self.htempFilterText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.htempFilterText.insert(tk.END,self.c.htempFilter)
        self.htempFilterText.grid(row=12,column=1,columnspan=1,sticky=tk.W)
        
        self.hlenFilterLabel=tk.Text(self.OptionsWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.hlenFilterLabel.grid(row=13,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.hlenFilterLabel,'Host Length Min (0 - 100):')
        self.hlenFilterText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.hlenFilterText.insert(tk.END,self.c.hlenFilter)
        self.hlenFilterText.grid(row=13,column=1,columnspan=1,sticky=tk.W)
        
        self.vtempFilterLabel=tk.Text(self.OptionsWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.vtempFilterLabel.grid(row=14,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.vtempFilterLabel,'Viral TM Min (C) (0 - 80):')
        self.vtempFilterText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.vtempFilterText.insert(tk.END,self.c.vtempFilter)
        self.vtempFilterText.grid(row=14,column=1,columnspan=1,sticky=tk.W)
        
        self.vlenFilterLabel=tk.Text(self.OptionsWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.vlenFilterLabel.grid(row=15,column=0,columnspan=1,sticky=tk.W)
        self.changeLabel(self.vlenFilterLabel,'Viral Length Min (0 - 100):')
        self.vlenFilterText=tk.Text(self.OptionsWin,height=1,width=5,state=tk.NORMAL)
        ...
        self.vlenFilterText.insert(tk.END,self.c.vlenFilter)
        self.vlenFilterText.grid(row=15,column=1,columnspan=1,sticky=tk.W)
        
        self.useBasicLabel=tk.Text(self.OptionsWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.useBasicLabel.grid(row=16,column=0,columnspan=1,sticky=tk.W)
        ...
        self.bt2locLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.bt2locLabel,'Bowtie2 Directory:')
        self.bt2locLabel.grid(row=0,column=0,columnspan=2,sticky=tk.W)
        self.bttn2=tk.Button(self.NewWin,text = '. . .', command=self.setBT2Folder)
        self.bttn2.grid(row=1,column=0,sticky=tk.W,ipadx=3)
        self.bt2Text=tk.Text(self.NewWin,height=1,width=70,state=tk.DISABLED,background=self.defaultbg)
        self.changeLabel(self.bt2Text,self.c.Bowtie2Folder)
        self.bt2Text.grid(row=1,column=1,sticky=tk.W,columnspan=4)
        self.vrfLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.vrfLabel,'Viral Reference *.fa:')
        self.vrfLabel.grid(row=2,column=0,columnspan=2,sticky=tk.W)
        self.bttn3=tk.Button(self.NewWin,text = '. . .', command=self.setViralRefFa)
        self.bttn3.grid(row=3,column=0,sticky=tk.W,ipadx=3)
        self.vrf=tk.Text(self.NewWin,height=1,width=70,state=tk.DISABLED,background=self.defaultbg)
        self.changeLabel(self.vrf,self.c.ViralRefFa)
        self.vrf.grid(row=3,column=1,sticky=tk.W,columnspan=4)
        self.vddLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.vddLabel,'Viral Index Directory:')
        self.vddLabel.grid(row=4,column=0,columnspan=2,sticky=tk.W)
        self.bttn4=tk.Button(self.NewWin,text = '. . .', command=self.setViralRefFolder)
        self.bttn4.grid(row=5,column=0,sticky=tk.W,ipadx=3)
        self.vdd=tk.Text(self.NewWin,height=1,width=70,state=tk.DISABLED,background=self.defaultbg)
        self.changeLabel(self.vdd,self.c.ViralRefFolder)
        self.vdd.grid(row=5,column=1,sticky=tk.W,columnspan=4)
        self.vpLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.vpLabel,'Viral Bowtie2 Prefix:')
        self.vpLabel.grid(row=6,column=0,columnspan=2,sticky=tk.W)
        self.vp=tk.Text(self.NewWin,height=1,width=70,state=tk.NORMAL)
        self.vp.insert(tk.INSERT,self.c.viralPrefix)
        self.vp.bind('<Return>',self.noReturn)
        self.vp.bind('<space>',self.noReturn)
        self.vp.grid(row=7,column=1,sticky=tk.W,columnspan=4)
        self.vpbttn=tk.Button(self.NewWin,text = 'Build', command=lambda: self.c.askbuildBowtie2ViralIndex(self.vp.get(1.0,'end-1c')))
        self.vpbttn.grid(row=7,column=0,sticky=tk.W)
        self.hrfLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.hrfLabel,'Host Reference *.fa:')
        self.hrfLabel.grid(row=8,column=0,columnspan=2,sticky=tk.W)
        self.bttn5=tk.Button(self.NewWin,text = '. . .', command=self.setHostRefFa)
        self.bttn5.grid(row=9,column=0,sticky=tk.W,ipadx=3)
        self.hrf=tk.Text(self.NewWin,height=1,width=70,state=tk.DISABLED,background=self.defaultbg)
        self.changeLabel(self.hrf,self.c.HostRefFa)
        self.hrf.grid(row=9,column=1,sticky=tk.W,columnspan=4)
        self.hddLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.hddLabel,'Host Index Directory:')
        self.hddLabel.grid(row=10,column=0,columnspan=2,sticky=tk.W)
        self.bttn6=tk.Button(self.NewWin,text = '. . .', command=self.setHostRefFolder)
        self.bttn6.grid(row=11,column=0,sticky=tk.W,ipadx=3)
        self.hdd=tk.Text(self.NewWin,height=1,width=70,state=tk.DISABLED,background=self.defaultbg)
        self.changeLabel(self.hdd,self.c.HostRefFolder)
        self.hdd.grid(row=11,column=1,sticky=tk.W,columnspan=4)
        self.hpLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.hpLabel,'Host Bowtie2 Prefix:')
        self.hpLabel.grid(row=12,column=0,columnspan=2,sticky=tk.W)
        self.hp=tk.Text(self.NewWin,height=1,width=70,state=tk.NORMAL)
        self.hp.insert(tk.INSERT,self.c.HostPrefix)
        self.hp.grid(row=13,column=1,sticky=tk.W,columnspan=4)
        self.hp.bind('<Return>',self.noReturn)
        self.hp.bind('<space>',self.noReturn)
        self.hpbttn=tk.Button(self.NewWin,text = 'Build', command=lambda: self.c.askbuildBowtie2HostIndex(self.hp.get(1.0,'end-1c')))
        self.hpbttn.grid(row=13,column=0,sticky=tk.W)

        self.hgtfLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)
        self.changeLabel(self.hgtfLabel,'Host GTF File:')
        self.hgtfLabel.grid(row=14,column=0,columnspan=2,sticky=tk.W)
        self.hgtf=tk.Text(self.NewWin,height=1,width=70,state=tk.DISABLED,background=self.defaultbg)
        self.changeLabel(self.hgtf,'None')
        self.hgtf.grid(row=15,column=1,sticky=tk.W,columnspan=4)
        self.hgtfbttn=tk.Button(self.NewWin,text = '. . .', command=self.setHostGTF)
        self.hgtfbttn.grid(row=15,column=0,sticky=tk.W,ipadx=3)
        
        self.outLabel=tk.Text(self.NewWin,height=1,width=30,state=tk.DISABLED,relief=tk.FLAT,background=self.defaultbg)        
        self.changeLabel(self.outLabel,'Output Directory:')
        self.outLabel.grid(row=16,column=0,columnspan=2,sticky=tk.W)
        self.bttn7=tk.Button(self.NewWin,text = '. . .', command=self.setOutputDir)
        self.bttn7.grid(row=17,column=0,sticky=tk.W,ipadx=3)
        self.out=tk.Text(self.NewWin,height=1,width=70,state=tk.DISABLED,background=self.defaultbg)
        self.changeLabel(self.out,self.c.outDir)
        self.out.grid(row=17,column=1,sticky=tk.W,columnspan=4)
    ...
    def selectSplitFiles(self, event):
        ...
        self.selectSplitFiles_lb = tk.Label(self.selectSplitFiles_Window)
        self.selectSplitFiles_lb["text"] = " Specify file number: n1, n2, ... "
        self.selectSplitFiles_lb.grid(row = 0, column = 0)
        # file numbers
        self.selectSplitFiles_Numbers = tk.Entry(self.selectSplitFiles_Window, width=120)
        self.selectSplitFiles_Numbers.grid(row = 1, column = 0)
        
        self.selectSplitFiles_btn = tk.Button(self.selectSplitFiles_Window, text=" OK ", \
    ...
    def openWeb(self):
        result = messagebox.askyesno("ChimericSeq","ChimericSeq Version "+self.releaseVersion+"\n"+
                            "For complete ChimericSeq information, Click 'Yes' to visit ChimericSeq web page.")
        if result == True:
    ...
    def selectBatchToggle(self):
        self.changeLabel(self.fastqBox,"")
        self.c.resetLargeFileProcess()
        self.c.batchmode = True
        self.selectDirButton.invoke()
        if self.selectDirVar.get() != 1:
            self.selectDirButton.invoke()
        if self.bttnStart["text"] == 'Start Run':
            btn_text.set('Split and Run Batch Files')
        elif self.bttnStart['text'] == 'Split and Run Batch Files':
            btn_text.set('Start Run')
        # self.printToLog(str(self.selectDirVar.get()))
        # self.printToLog(str(self.selectBatchVar.get()))
        return
    ...
    def getReads(self):
        readNames = []
        if self.c.batchmode and self.c.batchindex == -1:
            reads = self.c.getBatchReads()
            if len(reads) < 1:
                self.STOP = True
                return
            else:
                readNames += reads
            self.printToLog(str(readNames))
            # if self.c.batchindex != self.c.batchfileslength - 1:
            #     self.c.batchindex += 1
            #     self.c.batchfilename = self.c.batchfiles[self.c.batchindex]
            #     self.printToLog('moving on to ' + self.c.batchfilename)
            #     self.c.getBatchReads()
        else:
    ...
    def entry_Leave(self, key, minimum, maximum, average):
        ...
        try:
            value=int(data)
            if value < minimum:
                self.currentConfigEntry.delete(1.0, tk.END)
    ...
    def mh_stringency_Enter(self, x):
        self.currentConfigEntry=self.OptionsWin.focus_get()
        self.mh_stringency1.delete(1.0, tk.END)
        ...
        if self.mh_stringency.get(1.0,'end-1c').strip() == "":
            ...
            self.mh_stringency.delete(1.0, tk.END)
            self.mh_stringency.insert(1.0, self.c.microhomologystringency)
        return
