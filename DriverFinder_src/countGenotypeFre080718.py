...
#def count_Genotype(parent, result_full_path, sample_folder_full_path, sample_folder):
def count_Genotype(parent, fileSelected, fullPath):
    samfile_in = fullPath
    samfile_out = fullPath.replace(".sam", "[noH].sam")
    #remove header
    command0 = "samtools view -Sq 1 " + samfile_in + ">" + samfile_out
    #command0 = "samtools view -S "+ samfile_in + ">" + samfile_out
    os.system(command0)

    #samfile_in = samfile_out
    #samfile_out = samfile_out.replace("noH", "noZ")
    #command1 = "samtools view -Sq 0 " + samfile_in + ">" + samfile_out
    #os.system(command1)
    #read lines
    plines = 0
